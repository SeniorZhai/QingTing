  ...
  implements IDataSource
{
  ...
  private boolean deleteProgramNodes(DataCommand paramDataCommand)
  {
    ...
    try
    {
      paramDataCommand = "delete from programNodes" + " where cid = '" + i + "'";
      DBManager.getInstance().getWritableDB("programNodes").execSQL(paramDataCommand);
      return true;
    }
  ...
  private boolean insertProgramNodes(DataCommand paramDataCommand)
  {
    ...
    if ((paramDataCommand == null) || (paramDataCommand.size() == 0))
      return false;
    localObject = DBManager.getInstance().getWritableDB("programNodes");
    try
    {
      ...
      while (i < paramDataCommand.size())
      {
        ...
        ((SQLiteDatabase)localObject).execSQL("insert into programNodes(cid,pid,dw,programNode) values(?, ?, ?, ?)", new Object[] { Integer.valueOf(j), Integer.valueOf(((ProgramNode)localNode).uniqueId), Integer.valueOf(k), str });
        i += 1;
      }
  ...
  private boolean updateProgramNodes(DataCommand paramDataCommand)
  {
    ...
    localObject1 = DBManager.getInstance().getWritableDB("programNodes");
    try
    {
      ...
      {
        long l = DatabaseUtils.queryNumEntries((SQLiteDatabase)localObject1, "programNodes");
        Log.d("ProgramNodesDS", "sym:数据库缓存节目单数量:" + l);
        if (l > 10000L)
        {
          localObject2 = ((SQLiteDatabase)localObject1).query(true, "programNodes", new String[] { "cid" }, null, null, null, null, null, null);
          ...
          while (((Iterator)localObject2).hasNext())
          {
            ...
            if (DatabaseUtils.queryNumEntries((SQLiteDatabase)localObject1, "programNodes", "cid=?", new String[] { localObject3 }) < 300L)
            {
              Log.d("ProgramNodesDS", String.format("sym:数据库删除id为%s的缓存", new Object[] { localObject3 }));
              ((SQLiteDatabase)localObject1).delete("programNodes", "cid=?", new String[] { localObject3 });
            }
          }
        }
      }
      ((SQLiteDatabase)localObject1).execSQL("delete from programNodes" + " where cid = '" + j + "'");
      ...
      while ((i < paramDataCommand.size()) && (i < k))
      {
        ...
        ((SQLiteDatabase)localObject1).execSQL("insert into programNodes(cid,pid,dw,programNode) values(?, ?, ?, ?)", new Object[] { Integer.valueOf(j), Integer.valueOf(((ProgramNode)localObject3).uniqueId), Integer.valueOf(0), str });
        i += 1;
      }
  ...
  public IDataToken doCommand(DataCommand paramDataCommand, IDataRecvHandler paramIDataRecvHandler)
  {
    ...
    if (paramIDataRecvHandler.equalsIgnoreCase("insertdb_program_node"))
      return doInsertCommand(paramDataCommand);
    if (paramIDataRecvHandler.equalsIgnoreCase("getdb_program_node"))
      return doAcquireCommand(paramDataCommand);
    if (paramIDataRecvHandler.equalsIgnoreCase("deletedb_program_node"))
      return doDeleteCommand(paramDataCommand);
    if (paramIDataRecvHandler.equalsIgnoreCase("updatedb_program_node"))
