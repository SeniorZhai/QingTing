...
public class ReserveView extends ViewGroupViewImpl
  implements IEventHandler, IMotionHandler
{
  ...
  private ReserveListView mListView;
  private MiniPlayerView mPlayerView;
  private MotionController motionController;
  private final ViewLayout standardLayout = ViewLayout.createViewLayoutWithBoundsLT(720, 1200, 720, 1200, 0, 0, ViewLayout.FILL);

  public ReserveView(Context paramContext)
  {
    super(paramContext);
    try
    {
      setBackgroundColor(SkinManager.getBackgroundColor());
      this.mEmptyView = new EmptyTipsView(paramContext, 2);
      addView(this.mEmptyView);
      this.mListView = new ReserveListView(paramContext);
  ...
  private void deleteSelected(List<Object> paramList)
  {
    if ((paramList != null) && (paramList.size() > 0))
    {
      ...
      while (i >= 0)
      {
        InfoManager.getInstance().root().mPersonalCenterNode.reserveNode.cancelReserve(((ReserveNode)paramList.get(i)).uniqueId);
        i -= 1;
      }
    }
 ...
 * Qualified Name:     fm.qingting.qtradio.view.personalcenter.reserve.ReserveView
