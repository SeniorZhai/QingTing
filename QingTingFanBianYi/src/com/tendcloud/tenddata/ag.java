package com.tendcloud.tenddata;

import android.content.Context;
import dalvik.system.DexClassLoader;
import java.io.File;
import java.net.URL;

public class ag
{
  private static final String c = "td-cache";
  private static final String d = "td-%.zip";
  private static final String e = "td.tmp-";
  private static final String f = "pref_longtime";
  private static final String g = "td.sdk.digest.";
  private static final byte[] m = { -57, 9, -109, 31, 108, -13, -36, 34, -71, 6, -27, -77, -64, -39, 104, 24, -69, -86, -101, -20, -45, 83, 117, 85, 104, -59, -39, -93, 93, 44, 6, 66, 55, 71, -104, 118, -115, 88, 97, 27, 127, -83, -61, 50, 3, -34, -37, -110, 56, -78, 72, 28, 55, 98, -64, 87, 52, -75, 93, -27, 64, -94, 78, 80, -96, 77, 62, 42, -86, -83, 24, -97, 90, 24, -92, 94, -34, 124, -75, -111, 13, 35, 84, 69, -24, -73, -60, 116, 35, -71, 90, 61, -54, 62, -85, 71, -13, 108, -1, -53, -57, -102, 78, 78, 93, -122, -27, -97, -92, -38, -114, -69, -119, 61, 119, 116, -95, 39, 78, -52, -68, 66, 105, -104, 27, -91, 72, -50, 18, -110, 9, 7, -73, 11, -10, -81, 88, 85, 61, -34, -6, 52, -123, 76, 75, -81, -111, -121, 8, 32, 73, 18, 32, 83, 120, -58, -110, -30, 47, 120, 1, -113, -74, 73, -23, 33, -70, -110, -46, -105, -70, -94, 77, -118, 1, 111, 112, 23, 0, 55, 25, -49, -71, 17, 48, -10, 74, 101, 75, 107, 72, -19, 104, 48, -106, -12, 53, 63, -38, 116, -5, -104, 99, -81, 86, -93, 80, -103, 13, -6, -110, -83, -97, 107, 51, -125, 46, 124, -51, -111, 2, 52, 96, 116, -80, -76, 107, 24, -109, 100, 34, -98, 37, -112, -93, 35, 57, 68, 55, -114, -36, -111, -57, -26, 32, 93, 117, 67, -51, 115, 2, -105, -40, 111, 59, 36, 1, 100, -11, -17, 8, 64, 44, -42, 50, -44, 57, 11, 91, -5, 64, -92, 102, 35, -83, -5, -4, 71, 16, 53, 87, -89, 32, 46, 91, -5, -54, 15, -29, 43, 36, -22, 122, 99, -10, -68, -115, 52, 91, -42, 6, -110, 31, 115, 112, 14, -77, 120, -94, 126, 119, -86, 0, -123, -107, 82, -13, 34, 95, 87, 18, -23, -8, -43, -111, -41, 50, -47, 55, -3, -19, -12, -42, -124, -100, -29, -113, 26, 84, -74, -110, -51, -30, -8, -114, -42, 115, 76, -87, 92, 82, 47, 102, 33, 123, 125, 24, 4, -26, -69, -46, 31, -89, -114, -69, 50, -88, -64, -32, 64, 40, 11, 83, -43, 93, -101, -84, -104, 47, -117, 83, 98, 79, -30, 107, -3, -88, 82, -39, 118, -94, -51, 80, 55, -90, 64, -124, -111, -108, 31, -32, -60, 19, -48, -59, 59, -111, 53, 25, 41, 72, 15, -117, 80, 91, -119, -120, -93, 119, 124, 38, 24, -113, 83, -94, 122, 127, 58, 67, 11, 28, -55, -68, -108, -75, -52, 60, 80, -41, -105, 96, -77, 90, -30, -20, -20, -101, -119, 114, -85, 115, -7, -97, -7, -76, -53, 116, 85, -98, -54, -126, 36, 60, 104, -71, -94, -66, -128, -88, -10, -5, 76, -6, 68, 36, 13, 44, -91, 86, -5, 23, 101, -125, 13, -65, -76, 72, -6, -26, 107, -118, 4, 80, -27, 103, 38, 45, 38, 36, -10, 119, 124, -61, 73, 47, 5, 35, -47, -113, 114, 0, 45, 99, -99, 120, 83, 75, 56, -102, 22, -113, 19, -35, 113, 112, 33, 77, -32, 105, -66, -52, 124, -90, -61, -55, 54, -103, -26, 126, -92, -19, 101, -124, -90, 82, 68, 91, 13, -51, 91, -111, -40, 67, 38, -13, 59, 114, -113, -46, -71, -119, -74, 112, 78, -23, 7, -112, -65, 58, -3, -35, 115, 20, -113, 55, 92, -63, -105, 59, -100, 38, 108, 75, -114, -79, -8, 95, -69, -33, -108, -88, -16, -2, -91, -32, -52, -56, -41, -7, -53, -34, -72, -76, -94, 60, 39, -52, 14, 114, -113, -79, 51, -37, 66, 46, -107, -24, -128, 44, 77, 98, -23, 90, -101, 104, -26, -84, 2, -80, 68, -98, 98, -87, -113, -58, 75, 52, 74, 50, 78, -78, -91, 117, 29, -3, -18, 21, -38, 104, 7, 123, 66, 76, -72, -118, -73, -7, 40, -23, -125, -85, 53, 45, 4, 42, -99, 116, 29, -43, -58, 81, 21, -122, 99, 12, 42, -3, 50, 72, -103, 65, 59, -112, 98, 116, -4, 37, 119, 80, -58, 93, -77, 27, -92, -90, 126, -63, -126, 45, 96, -35, 25, -120, 20, -23, 49, 10, 119, 54, 2, 90, 106, 101, 21, -125, -26, 26, 103, 28, -73, -110, 44, -109, 4, 27, -73, -74, 44, 111, -71, 97, -16, -120, -69, 69, -98, 125, 84, -125, -122, 56, -55, -101, 18, -102, -87, 41, -103, 63, -62, 52, -3, 53, 28, 37, 49, -88, 104, 104, -113, 125, -41, -70, -50, -12, 12, 84, -3, -88, -31, -48, 55, 2, 97, 19, 123, 102, 71, 118, -65, 13, -68, -103, 23, -102, 66, -62, -26, 20, 95, 12, -128, 115, 9, 43, 116, -106, -44, -48, 92, 8, 119, 98, -35, 43, 24, -1, -20, 1, 80, 39, -102, -118, 57, 100, 88, -95, -113, -75, -16, 9, 91, 67, -118, -4, -98, -62, 37, 14, 112, -66, -28, 59, 119, 0, 33, -50, -58, 98, 78, -2, -113, -73, -97, -110, -37, 51, -51, -58, 25, 58, 36, -10, 112, 21, 76, -39, 1, -64, -48, 92, 37, -39, 1, -22, -51, -121, 124, 13, 3, 33, 76, 93, 82, 110, 95, -105, 69, 79, 10, 112, 60, 50, -60, 70, -18, -75, 85, -9, -3, 36, -66, -90, 107, 71, 67, -26, -108, -101, -54, -81, 58, -69, -63, 99, 53, -51, 58, 86, -10, 39, -112, -15, 37, -56, 79, 86, -92, 102, -88, -70, -26, -11, -35, 20, 47, 69, -27, 58, -37, -116, -35, -47, 96, 81, 89, -63, -12, -44, -117, 63, 105, 24, 53, -25, 123, -45, 48, -92, -126, -83, -49, 35, -64, 125, -96, 98, -5, -51, 78, -48, -91, 127, -119, -79, 2, 104, -70, -27, -61, -25, 55, 82, -107, -8, 9, -28, -110, 30, -1, -9, 17, 7, 104, 60, -61, 84, 92, 53, -30, -74, -75, 88, -34, -109, 63, -71, -12, -56, 96, -37, -25, -28, 60, 91, 37, 14, -4, 17, 63, 116, -96, 117, 91, -84, 118, -58, -21, 88, 9, -49, 47, -99, 6, -25, -75, 127, -20, 49, -41, 117, 10, 54, 86, 92, -57, 97, -7, -25, -77 };
  private static final int[] n = { 48, 174, 179, 57, 117, 158, 111, 77, 5, 190, 185, 241, 57, 21, 216, 178, 37, 2, 95, 166 };
  private static final int[] o = { 104, 169, 91, 102, 66, 239, 22, 20, 116, 6, 95, 8, 9, 28, 206, 160, 123, 152, 33, 126 };
  u a;
  Context b;
  String h;
  String i;
  String j;
  String k;
  volatile boolean l = false;

  private u a(String paramString)
  {
    try
    {
      this.i = "td-%.zip".replace("%", this.h);
      this.j = ("td.tmp-" + this.h);
      this.k = ("td.sdk.digest." + this.h);
      File localFile1 = this.b.getDir("td-cache", 0);
      File localFile2 = new File(localFile1, this.i);
      if (a(localFile2))
      {
        paramString = (u)new DexClassLoader(localFile2.getAbsolutePath(), localFile1.getAbsolutePath(), null, ag.class.getClassLoader()).loadClass(paramString).newInstance();
        return paramString;
      }
    }
    catch (Throwable paramString)
    {
    }
    return null;
  }

  private void a(u paramu)
  {
    Object localObject1 = new URL(paramu.c());
    Object localObject2 = ((URL)localObject1).getHost();
    int i2 = ((URL)localObject1).getPort();
    int i1 = i2;
    if (i2 == -1)
      i1 = ((URL)localObject1).getDefaultPort();
    localObject1 = new ad(new String(x.a(x.a(n, o), m)), (String)localObject2, i1);
    localObject2 = ((ad)localObject1).a(paramu.c());
    File localFile;
    if (!x.a((String)localObject2))
    {
      localObject2 = ((String)localObject2).trim();
      paramu.a((String)localObject2);
      if (!((String)localObject2).equalsIgnoreCase(paramu.a()))
      {
        localFile = this.b.getDir("td-cache", 0);
        localObject2 = new File(localFile, this.j);
        localObject1 = ((ad)localObject1).a(paramu.d(), (File)localObject2);
        if (localObject1 == null);
      }
    }
    try
    {
      localFile = new File(localFile, this.i);
      localFile.delete();
      ((File)localObject2).renameTo(localFile);
      aa.a(this.b, "pref_longtime", this.k, (String)localObject1);
      paramu.e();
      return;
    }
    catch (Exception paramu)
    {
    }
  }

  // ERROR //
  private boolean a(File paramFile)
  {
    // Byte code:
    //   0: new 452	java/io/FileInputStream
    //   3: dup
    //   4: aload_1
    //   5: invokespecial 455	java/io/FileInputStream:<init>	(Ljava/io/File;)V
    //   8: astore_1
    //   9: ldc_w 457
    //   12: invokestatic 463	java/security/MessageDigest:getInstance	(Ljava/lang/String;)Ljava/security/MessageDigest;
    //   15: astore_2
    //   16: sipush 1024
    //   19: newarray byte
    //   21: astore_3
    //   22: aload_1
    //   23: aload_3
    //   24: invokevirtual 467	java/io/FileInputStream:read	([B)I
    //   27: istore 4
    //   29: iload 4
    //   31: iconst_m1
    //   32: if_icmpeq +21 -> 53
    //   35: aload_2
    //   36: aload_3
    //   37: iconst_0
    //   38: iload 4
    //   40: invokevirtual 471	java/security/MessageDigest:update	([BII)V
    //   43: goto -21 -> 22
    //   46: astore_2
    //   47: aload_1
    //   48: invokevirtual 474	java/io/FileInputStream:close	()V
    //   51: iconst_0
    //   52: ireturn
    //   53: aload_2
    //   54: invokevirtual 478	java/security/MessageDigest:digest	()[B
    //   57: invokestatic 481	com/tendcloud/tenddata/x:a	([B)Ljava/lang/String;
    //   60: astore_2
    //   61: aload_0
    //   62: getfield 334	com/tendcloud/tenddata/ag:b	Landroid/content/Context;
    //   65: ldc 17
    //   67: aload_0
    //   68: getfield 332	com/tendcloud/tenddata/ag:k	Ljava/lang/String;
    //   71: ldc_w 483
    //   74: invokestatic 486	com/tendcloud/tenddata/aa:b	(Landroid/content/Context;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;
    //   77: aload_2
    //   78: invokevirtual 490	java/lang/String:equals	(Ljava/lang/Object;)Z
    //   81: istore 5
    //   83: aload_1
    //   84: invokevirtual 474	java/io/FileInputStream:close	()V
    //   87: iload 5
    //   89: ireturn
    //   90: astore_1
    //   91: iload 5
    //   93: ireturn
    //   94: astore_2
    //   95: aconst_null
    //   96: astore_1
    //   97: aload_1
    //   98: invokevirtual 474	java/io/FileInputStream:close	()V
    //   101: aload_2
    //   102: athrow
    //   103: astore_1
    //   104: iconst_0
    //   105: ireturn
    //   106: astore_1
    //   107: goto -6 -> 101
    //   110: astore_2
    //   111: goto -14 -> 97
    //   114: astore_1
    //   115: aconst_null
    //   116: astore_1
    //   117: goto -70 -> 47
    //
    // Exception table:
    //   from	to	target	type
    //   9	22	46	java/lang/Exception
    //   22	29	46	java/lang/Exception
    //   35	43	46	java/lang/Exception
    //   53	83	46	java/lang/Exception
    //   83	87	90	java/io/IOException
    //   0	9	94	finally
    //   47	51	103	java/io/IOException
    //   97	101	106	java/io/IOException
    //   9	22	110	finally
    //   22	29	110	finally
    //   35	43	110	finally
    //   53	83	110	finally
    //   0	9	114	java/lang/Exception
  }

  // ERROR //
  public u a(Context paramContext, String paramString1, String paramString2, Class paramClass1, Class paramClass2, String paramString3)
  {
    // Byte code:
    //   0: aload_0
    //   1: monitorenter
    //   2: aload_0
    //   3: getfield 495	com/tendcloud/tenddata/ag:a	Lcom/tendcloud/tenddata/u;
    //   6: ifnonnull +82 -> 88
    //   9: aload_0
    //   10: aload_1
    //   11: invokevirtual 499	android/content/Context:getApplicationContext	()Landroid/content/Context;
    //   14: putfield 334	com/tendcloud/tenddata/ag:b	Landroid/content/Context;
    //   17: aload_0
    //   18: aload_3
    //   19: putfield 309	com/tendcloud/tenddata/ag:h	Ljava/lang/String;
    //   22: aload_0
    //   23: monitorenter
    //   24: new 501	java/lang/Thread
    //   27: dup
    //   28: new 503	com/tendcloud/tenddata/w
    //   31: dup
    //   32: aload_0
    //   33: aload 6
    //   35: invokespecial 506	com/tendcloud/tenddata/w:<init>	(Lcom/tendcloud/tenddata/ag;Ljava/lang/String;)V
    //   38: invokespecial 509	java/lang/Thread:<init>	(Ljava/lang/Runnable;)V
    //   41: invokevirtual 512	java/lang/Thread:start	()V
    //   44: aload_0
    //   45: ldc2_w 513
    //   48: invokevirtual 518	java/lang/Object:wait	(J)V
    //   51: aload_0
    //   52: monitorexit
    //   53: aload_0
    //   54: getfield 495	com/tendcloud/tenddata/ag:a	Lcom/tendcloud/tenddata/u;
    //   57: ifnonnull +20 -> 77
    //   60: aload_0
    //   61: iconst_1
    //   62: putfield 299	com/tendcloud/tenddata/ag:l	Z
    //   65: aload_0
    //   66: aload 5
    //   68: invokevirtual 372	java/lang/Class:newInstance	()Ljava/lang/Object;
    //   71: checkcast 374	com/tendcloud/tenddata/u
    //   74: putfield 495	com/tendcloud/tenddata/ag:a	Lcom/tendcloud/tenddata/u;
    //   77: aload_0
    //   78: getfield 495	com/tendcloud/tenddata/ag:a	Lcom/tendcloud/tenddata/u;
    //   81: aload_1
    //   82: aload_2
    //   83: invokeinterface 521 3 0
    //   88: aload_0
    //   89: getfield 495	com/tendcloud/tenddata/ag:a	Lcom/tendcloud/tenddata/u;
    //   92: astore_1
    //   93: aload_0
    //   94: monitorexit
    //   95: aload_1
    //   96: areturn
    //   97: astore_1
    //   98: aload_0
    //   99: monitorexit
    //   100: aload_1
    //   101: athrow
    //   102: astore_1
    //   103: aload_0
    //   104: monitorexit
    //   105: aload_1
    //   106: athrow
    //   107: astore_3
    //   108: goto -57 -> 51
    //
    // Exception table:
    //   from	to	target	type
    //   24	51	97	finally
    //   51	53	97	finally
    //   98	100	97	finally
    //   2	24	102	finally
    //   53	77	102	finally
    //   77	88	102	finally
    //   88	93	102	finally
    //   100	102	102	finally
    //   24	51	107	java/lang/InterruptedException
  }
}

/* Location:           C:\Users\User\dex2jar-2.0\dex\qting\classes-dex2jar.jar
 * Qualified Name:     com.tendcloud.tenddata.ag
 * JD-Core Version:    0.6.2
 */